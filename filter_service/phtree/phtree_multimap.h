...
class PhTreeMultiMap {
  ...
  public:
    ...
    template <typename... Args>
    std::pair<v16::Entry<DIM, BUCKET, scalar_64_t>*, bool> emplace_e(const Key& key, Args&&... args) {
        auto* outer_entry = tree_.try_emplace_e(converter_.pre(key)).first;
        auto bucket_iter = outer_entry->GetValue().emplace(std::forward<Args>(args)...);
        size_ += bucket_iter.second ? 1 : 0;
        return {outer_entry, bucket_iter.second};
    }
